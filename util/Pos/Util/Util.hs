...
-- | Subtract one number from another (like 'subtract'), but return
-- 'Nothing' if the result would've been negative.
--
-- > (subtractMay 4) 10
-- Just 6
--
-- > (subtractMay 2) 1
-- Nothing
subtractMay :: (Num a, Ord a) => a -> a -> Maybe a
subtractMay b a | a >= b    = Just (a - b)
                | otherwise = Nothing
